...
public class CraftTutorial : MonoBehaviour {
		...
		void Start () {
			...
			rect = GameObject.Find ("Main Camera").GetComponent<MouseToTouchTutorial> ().rect;
		...
		public void ShowRecipe(Recipe r,string action)
		{
			...
			MouseToTouchTutorial.pintando = false;
		...
		public void Refresh()
		{
		Fungus.Flowchart.BroadcastFungusMessage ("Bloque9");

			foreach (GameObject g in Nums) {
				g.SetActive (false);
				g.GetComponent<Text> ().color = new Color(0.19f,0.19f,0.19f);
				GameObject.Find ("Main Camera").GetComponent<MouseToTouchTutorial> ().RestartTable ();
			}
		}
		...
		public void CorrectRect(int num){
			...
			case 0:
				if (num.Equals (CurrentRecipe.roj+CurrentRecipe.nar+CurrentRecipe.lila)) {
					...
					MouseToTouchTutorial.correcto=true;
			...
			case 1:
			int i = GameObject.Find ("Main Camera").GetComponent<MouseToTouchTutorial> ().Corroborar(colors [3]);
			...
			case 2:
			int verde = GameObject.Find ("Main Camera").GetComponent<MouseToTouchTutorial> ().Corroborar (colors [4]);
			int lila = GameObject.Find ("Main Camera").GetComponent<MouseToTouchTutorial> ().Corroborar (colors [5]);
