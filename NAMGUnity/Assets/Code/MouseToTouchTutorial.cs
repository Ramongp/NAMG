...
public class MouseToTouchTutorial : MonoBehaviour {
	...
	bool bloque6,bloque7,bloque8;
		...
		if ((TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE6))&& (bloque6)) {
			bloque6 = false;

			for (int i = 0; i <= numFilas; i++) {
				for (int u = 0; u <= numFilas; u++) {
					mat [i, u].g.GetComponent<MeshRenderer> ().material.color = Color.gray;
				}
			}
				for (int i = 2; i <= 7; i++) {
					for (int u = 2; u <= 6; u++) {
						...
						if((i.Equals(u))||((i.Equals(7)&&(u.Equals(6)))))
							{
							mat[i,u].g.GetComponentInChildren<Animator>().SetBool("Ver",true);
							}
					}
			}
		}

		if ((TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE7)) && (bloque7)) {
			bloque7 = false;
			for (int i = 8; i <= 9; i++) {
				for (int u = 6; u <= 7; u++) {
					mat [i, u].g.GetComponent<MeshRenderer> ().material.color = Color.white;
		...
		/*if ((TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE8)) && (bloque8)) {
			bloque8 = false;
			for (int i = 8; i <= 9; i++) {
				for (int u = 6; u <= 9; u++) {
					mat [i, u].g.GetComponent<MeshRenderer> ().material.color = Color.white;
					if ((i.Equals (8) && (u < 9)) || ((i.Equals (9)) && (u.Equals (i)))) {
						mat[i,u].g.GetComponentInChildren<Animator>().SetBool("Ver",true);
					}
				}
			}
				...
				if (Physics.Raycast (touchPosition, Vector3.forward, out hit2)) {
					if (hit2.collider.tag.Equals ("Quad")) {
						if (rect.activeSelf.Equals (true)) {
							...
							//Corroborar area
						CraftTutorial g = GameObject.Find ("CreadorRecetas").GetComponent<CraftTutorial> ();
						...
						if (correcto) {
							...
							if (TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE8)) {
								Fungus.Flowchart.BroadcastFungusMessage ("Bloque8");
								crafteando = false;
							}
							if (TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE6)) {
								TutorialManager.CurrenState = TutorialManager.EstadosDeBatalla.BLOQUE7;
								for (int i = 2; i <= 7; i++) {
									for (int u = 2; u <= 6; u++) {
										if ((i.Equals (u)) || ((i.Equals (7) && (u.Equals (6))))) {
								...
								Fungus.Flowchart.BroadcastFungusMessage ("Bloque6");
								crafteando = false;
							}
							...
							if ((TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE7) && ((Fila * Columna).Equals (4)))) {
								Fungus.Flowchart.BroadcastFungusMessage ("Bloque7");
								crafteando = false;
							} else {
								Fungus.Flowchart.BroadcastFungusMessage ("BloqueEquivocado");
							}
		...
		public void Refresh()
		{
		GameObject.Find ("ArrowReset").GetComponent<Animator> ().SetBool ("Ver", false);
			if (crafteando) {
		...
		if (TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE7)) {
			for (int i = 8; i <= 9; i++) {
				for (int u = 6; u <= 7; u++) {
					mat [i, u].g.GetComponent<MeshRenderer> ().material.color = Color.white;
				}
		...
		if (TutorialManager.CurrenState.Equals (TutorialManager.EstadosDeBatalla.BLOQUE8)) {
			for (int i = 8; i <= 9; i++) {
				for (int u = 6; u <= 9; u++) {
					mat [i, u].g.GetComponent<MeshRenderer> ().material.color = Color.white;
					if ((i.Equals (9) && (u > 6)) || ((i.Equals (8)) && (u.Equals (6)))) {
						mat[i,u].g.GetComponentInChildren<Animator>().SetBool("Ver",true);
					}
				}
			}
